...
def bernoulli_(self, p=0.5):
    ...
    if isinstance(p, torch.Tensor):
        torch._check_tensor_all(
            (p >= 0) & (p <= 1),
            lambda: "bernoulli_ expects all probabilities to be in [0, 1], got out-of-range values",
        )
    else:
        torch._check(
            0 <= p <= 1,
...
def bernoulli_p(self, p=0.5, *, generator=None):
    ...
    if isinstance(p, torch.Tensor):
        torch._check_tensor_all(
            (p >= 0) & (p <= 1),
            lambda: "bernoulli expects all probabilities to be in [0, 1], got out-of-range values",
        )
    else:
        torch._check(
            0 <= p <= 1,
